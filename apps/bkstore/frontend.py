...
def get_selected_row(event): # the event parameter is a flag to highlight that this function is binded to an event and is necessary.
    ...
    selected_tuple=list1.get(index) # from the listbox, retrieve the row which corresponds to the obtained index from the event of clicking.
    e1.delete(0,tk.END) # clear e1
    e1.insert(tk.END,selected_tuple[1]) # insert title when the row is selected
    e2.delete(0,tk.END) # clear e2
    e2.insert(tk.END,selected_tuple[2]) # insert author in author entry widget when row is selected
    e3.delete(0,tk.END) # clear e3
    e3.insert(tk.END,selected_tuple[3]) # insert year in year entry widget when row is selected
    e4.delete(0,tk.END) # clear e4
    e4.insert(tk.END,selected_tuple[4]) # insert isbn in isbn entry widget when rows is selected


def view_command(): # func to iterate on each tuple inside obtained list from backend
...
def delete_command():
    backend.delete(selected_tuple[0]) # pass the first index (id) of the obtained tuple from get_selected_row()
...
list1.bind("<<ListboxSelect>>",get_selected_row) # bind the listbox to an condition/action (a selection of an item on the listbox), and select an effect to be performed when the action is met.
